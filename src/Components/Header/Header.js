...
import { categories, dataForTheMenu } from "../../Data";
...
const shoesOptions = shoes.map(x => {
  return (
    <MenuItem key={x.name} value={x.name}>
      {x.name}
    </MenuItem>
...
const bootsOptions = boots.map(x => {
  return (
    <MenuItem key={x.name} value={x.name}>
      {x.name}
    </MenuItem>
      ...
      >
        <Toolbar>
          <div className="left-part">
            ...
            >
              {}, {bootsOptions}, {shoesOptions}, {categoryOptions}
            </Select>

            <Button
              style={{ marginLeft: 20, whiteSpace: "nowrap",
                overflow: "hidden",
                textOverflow: "ellipsis",textalign: "center" ,fontsize:9,
    lineheight:1,
    padding:5,
